---
description: Remember a working note to Bettyjane memory (signed with the agent key)
allowed-tools: Bash(bun bin/bj.ts remember:*)
---

Mint the note below as a Bettyjane **agent memory** — a working memory signed
with the agent key — then report the resulting txid.

Run the bj CLI from the Bettyjane project root with the Bash tool:

`bun bin/bj.ts remember "<the note below>"`

(If the current directory is not the Bettyjane repo because the plugin is
installed elsewhere, run the same command against `$CLAUDE_PLUGIN_ROOT/bin/bj.ts`
instead.) Add `--namespace <name>` to remember into a named memory namespace. The
note is written permanently and publicly on chain.

Note to remember: $ARGUMENTS
